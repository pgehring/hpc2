...
void meshsplit(mesh* H, mesh_trans** metra, index anz_dom){
    ...
    index nelem  = H->nelem;
    index anz_nod_edge = sqrt(ncoord);
    index anz_nod_domedgex = (anz_nod_edge+anz_dom_edgex-1)/anz_dom_edgex;
    index anz_nod_domedgey = (anz_nod_edge+anz_dom_edgey-1)/anz_dom_edgey;
    index anz_nod_dom = anz_nod_domedgex*anz_nod_domedgey;
	...
	for(size_t k=0; k<ncoord; k++){
	    if(coord[2*k]>=(double)i/anz_dom_edgex 
		    && coord[2*k]<= (double)(i+1)/anz_dom_edgex){
		if(coord[2*k]==(double)i/anz_dom_edgex){
		    hilfsvek[k] = 10; 	//west
		} else if (coord[2*k]==(double)(i+1)/anz_dom_edgex){
		    hilfsvek[k] = 15;	//east
		} else {
		    hilfsvek[k] = 1;	//in einer der Domains
		}
	    }else{
