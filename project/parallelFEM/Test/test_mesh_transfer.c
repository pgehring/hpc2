...
bool testMeshData(MeshMapping ***globalMapping, MeshMapping *localMapping,
	...
	index *testElemBdryEdgeData = malloc(globalMesh->ncoord * sizeof(index));
	double *testCoordData = malloc(globalMesh->ncoord * sizeof(index));
	...
	for (int r=1; r<nof_processes; ++r){
	     ...
	     memset(testCoordData, 0.0, globalMesh->ncoord*sizeof(double));
	     // get the test data by communicating and compute RMSE
	     ...
	     memset(testElemBdryEdgeData, 0, globalMesh->ncoord*sizeof(index));
		 ...
		 nofData = 2*globalMapping[pCoords[0]][pCoords[1]]->localMesh->nfixed;
		 memset(testElemBdryEdgeData, 0, globalMesh->ncoord*sizeof(index));
	...
	MPI_Send(localMapping->localMesh->coord, localMapping->localMesh->ncoord,
		 MPI_DOUBLE, 0, 2, grid);

	// Send element data to root
	MPI_Send(localMapping->localMesh->elem, localMapping->localMesh->nelem,
		 MPI_LONG_LONG, 0, 3, grid);

	// Send boundary data to root
	MPI_Send(localMapping->localMesh->bdry,localMapping->localMesh->nbdry,
	...
	if (localMapping->localMesh->fixed){
	    MPI_Send(localMapping->localMesh->fixed,2* localMapping->localMesh->nfixed,
