...
void getReferenceMetaData(index *buffer, MeshMapping ***globalMapping, int *dims) {
    ...
    buffer[6] = globalMapping[dims[0]][dims[1]]->lMeshDimX;
    buffer[7] = globalMapping[dims[0]][dims[1]]->lMeshDimY;
}
...
bool testMeshMetadata(mesh *globalMesh, MeshMapping ***globalMapping,
    ...
    if (rank == 0) {
        printf("%-4s|(x,y)|%-8s|%-8s|%-8s|%-8s|%-8s|%-11s|%-8s|%-8s|%-5s|%-5s\n", "", "ncoord", "nelem", "nedges",
               "nbdry", "nfixed", "glbl ncoord", "lMeshDimX", "lMeshDimY", "RMSE", "pass");
        printf("----------------------------------------------------------------------------------\n");

        index refMetadata[8], tstMetadata[8];
        int procCoords[2];
        double rmse;
        bool resultCurrent;
        ...
        for (int r = 0; r < nof_processes; ++r) {
            ...
            printf("%-4s|(%d,%d)|%-8td|%-8td|%-8td|%-8td|%-8td|%-11td|%-8td|%-8td|%-5d|%-5s\n",
                   "ref", procCoords[0], procCoords[1], refMetadata[0], refMetadata[1],
                   refMetadata[2], refMetadata[3], refMetadata[4], refMetadata[5], refMetadata[6], refMetadata[7], 0, " ");
            ...
            if (r == 0) {
                ...
                tstMetadata[6] = localMapping->lMeshDimX;
                tstMetadata[7] = localMapping->lMeshDimY;
            } else {
            ...
            printf("%-4s|(%d,%d)|%-8td|%-8td|%-8td|%-8td|%-8td|%-11td|%-8td|%-8td|%-1.3lf|%-5s\n",
                   "tst", procCoords[0], procCoords[1], tstMetadata[0], tstMetadata[1],
                   tstMetadata[2], tstMetadata[3], tstMetadata[4], tstMetadata[5], tstMetadata[6], tstMetadata[7],
                   rmse, testRestultString);
        }
    } else {
        // Collect metadata of the local mapping
        index sendData[8] = {localMapping->localMesh->ncoord,
                             ...
                             localMapping->lMeshDimX,
                             localMapping->lMeshDimY};
