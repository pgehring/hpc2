...
mesh *mesh_alloc(index ncoord, index nelem, index nbdry) {
    mesh *M = reinterpret_cast<mesh*>(malloc(sizeof(mesh))); /* allocate the mesh struct */
    ...
    M->nedges = 0;
    M->nfixed = 0;
    M->coord = reinterpret_cast<double*>(malloc(ncoord * 2 * sizeof(double)));
    M->elem = reinterpret_cast<index*>(malloc(nelem * 7 * sizeof(index)));
    M->bdry = reinterpret_cast<index*>(malloc(nbdry * 4 * sizeof(index)));
    M->edge2no = NULL;
    M->fixed = NULL;
