...
void stima_laplace(double p1[2], double p2[2], double p3[2], index typ,
                   double m[3][3]) {
    int i, j;
...
void stima_laplace3(double p1[2], double p2[2], double p3[2], index typ,
    ...
    /* less operation by using the properties, symmetry and row_sum = col_sum =
     * 0
     */
    /* in total with computing d and mid, 20 'additions' and 18
     * 'multiplications'
     */
    dx[0] = fac * (d[0][0] * d[0][0] + d[0][1] * d[0][1]);
    dx[1] = fac * (d[1][0] * d[1][0] + d[1][1] * d[1][1]);
    ax[0] = ax[2] = fac * (d[0][0] * d[1][0] + d[0][1] * d[1][1]);
    ax[8] = 2.0 * ax[0];
    ax[1] = ax[5] = -(dx[0] + ax[0]);
    ax[6] = 2.0 * ax[1];
    ax[3] = ax[4] = -(dx[1] + ax[0]);
...
sed *sed_nz_pattern(mesh *M) {
    ...
    static int ai[9] = {0, 0, 1, 1, 2, 2, 3, 3, 4},
               aj[9] = {3, 5, 3, 4, 4, 5, 4, 5, 5};
    sed *S;
    ...
    w = reinterpret_cast<index*>(calloc(n, sizeof(index)));           /* get workspace */
...
index sed_buildS(mesh *M, sed *T) {
    ...
    if (!(T->x)) Tx = T->x = reinterpret_cast<double*>(calloc(Ti[n], sizeof(double)));
    if (!Tx) return (0);

    for (k = 0; k < nT; k++) {
        for (j = 0; j < 3; j++) ind[j] = Elem[7 * k + j];
        for (j = 3; j < 6; j++) ind[j] = nC + Elem[7 * k + j];
        stima_laplace3(Coord + 2 * ind[0], Coord + 2 * ind[1],
                       Coord + 2 * ind[2], Elem[7 * k + 7], dx, ax);
