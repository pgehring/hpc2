...
sed *sed_alloc(index n, index nzmax, index values) {
    sed *A = reinterpret_cast<sed*>(calloc(1, sizeof(sed))); /* allocate the cs struct */
    if (!A) return (NULL);           /* out of memory */
    A->n = n;                        /* define dimensions and nzmax */
    A->nzmax = nzmax = HPC_MAX(nzmax, n + 1);
    A->i = reinterpret_cast<index*>(malloc(nzmax * sizeof(index)));
    if (!A->i) return (NULL);
    A->i[A->n] = nzmax;
    A->x = values ? reinterpret_cast<double*>(malloc(nzmax * sizeof(double))) : NULL;
...
index sed_realloc(sed *A, index nzmax) {
    ...
    A->i = reinterpret_cast<index*>(hpc_realloc(A->i, nzmax, sizeof(index), &oki));
    if (A->x) A->x = reinterpret_cast<double*>(hpc_realloc(A->x, nzmax, sizeof(double), &okx));
    ok = (oki && okx);
    if (ok) A->nzmax = nzmax;
    return (ok);
}
