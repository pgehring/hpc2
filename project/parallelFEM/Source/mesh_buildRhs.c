#include "hpc.h"
void mesh_Neumann(double p1[2], double p2[2], index typ,
                  double (*fc)(double *, index), double m[3]) {
  int i;
...
void mesh_vol(double p1[2], double p2[2], double p3[2], index typ,
              double (*fc)(double *, index), double b[6]) {
  int j;
...
void mesh_buildRhs(const mesh *M, double *b, double (*fV)(double *, index),
  ...
  for (k = 0; k < nT; k++) {
    ...
    for (j = 3; j < 6; j++)
      ind[j] = nC + Elem[7 * k + j];
    mesh_vol(Coord + 2 * ind[0], Coord + 2 * ind[1], Coord + 2 * ind[2],
             Elem[7 * k + 6], fV, bx);
    for (j = 0; j < 6; j++)
      b[ind[j]] += bx[j];
  }
  ...
  for (k = 0; k < nB; k++) {
    if (Bdry[4 * k + 3]) {
      for (j = 0; j < 2; j++)
        ind[j] = Bdry[4 * k + j];
      ind[2] = nC + Bdry[4 * k + 2];
      mesh_Neumann(Coord + 2 * ind[0], Coord + 2 * ind[1], Bdry[4 * k + 3], fN,
                   bx);
      for (j = 0; j < 3; j++)
        b[ind[j]] += bx[j];
    }
