 ...
 *
 * @param[in] localMesh	Gitter des lokalen Teilgebiets als mesh_transfer struct.
 *
 * @return Zurückgeben einer allokierten SED Matrix.
 */
sed *sed_sm_pattern(mesh *localMesh) {
 ...
 * @param[out] dx Speicher für die Diagonalelemente.
 * @param[out] ax Speicher für die Nebendiagonalelemente.
 */
 ...
 * Funktion zur Berechnung des SED Matrix Speicherlayouts.
 *
 * @param localMesh Lokales Gitter als mesh_transfer.
 *
...
sed *sed_sm_build(mesh *localMesh) {
    ...
    if (!Ax) {
        printf("[E] Could not allocate stiffness matrix memory\n");
        abort();
    }
